...
class RecurrenceResult:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "candidate_fetch_passes": self.candidate_fetch_passes,
...
class SegmentRecurrenceClusterer:
    ...
    def from_candidates(
        ...
        *,
        similarity_method: str,
        threshold: float,
        candidate_fetch_passes: int = 1,
        min_occurrences: int = 2,
        limit: int | None = 100,
    ) -> RecurrenceResult:
        if candidate_fetch_passes < 1:
            raise ValueError("candidate_fetch_passes must be positive")
        ...
        limited = clusters if limit is None else clusters[:limit]
        return RecurrenceResult(
            method=_METHOD,
            similarity_method=similarity_method,
            threshold=threshold,
            candidate_edge_count=len(candidates),
            candidate_fetch_passes=candidate_fetch_passes,
            cluster_count=cluster_count,
            returned_cluster_count=len(limited),
            clusters=tuple(limited),
        )

    def _complete_similarity_candidates(
        self,
        *,
        threshold: float,
        shingle_size: int,
    ...
    ) -> tuple[tuple[NearDuplicateCandidate, ...], str, int]:
        """Retrieve the complete matched set without requiring an unbounded similarity API.

        The current similarity layer exposes a presentation limit. Recurrence clustering needs
        every matched edge, so the window doubles only when it fills. A non-full window proves
        that no matched edges were truncated by that request.
        """
        request_limit = 64
        passes = 0
        while True:
            result = self.similarity.find(
                threshold=threshold,
            ...
            if len(candidates) < request_limit:
                return candidates, result.method, passes
            possible_pairs = int(result.possible_all_pairs)
            if request_limit > possible_pairs:
                return candidates, result.method, passes
            request_limit = min(request_limit * 2, possible_pairs + 1)
    ...
    def find(
        ...
        *,
        ...
        candidates, similarity_method, passes = self._complete_similarity_candidates(
            threshold=threshold,
            shingle_size=shingle_size,
            min_shared_shingles=min_shared_shingles,
            max_shingle_frequency=max_shingle_frequency,
