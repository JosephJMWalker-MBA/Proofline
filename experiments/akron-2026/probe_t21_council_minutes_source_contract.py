#!/usr/bin/env python3
"""Probe Akron Public Access Council Meeting Minutes query metadata only.

This stage inherits the publisher-issued Public Access contract frozen by R1.T21
and requests only the keyword schema for the exact publisher query named
"Council Meeting Minutes" (query 175). It does not submit a document search,
dereference a document token, or assign an Eastwood outcome.
"""
...
SCHEMA = "proofline-akron-t21-council-minutes-source-contract/v1"
...
def select_minutes_query(source: dict) -> dict:
    ...
    if query.get("id") != QUERY_ID:
        raise ValueError("Council Meeting Minutes query ID diverged from frozen publisher metadata")
    if query.get("type") != "DocumentType":
        raise ValueError("Council Meeting Minutes must remain a DocumentType query")
    return query
...
def main() -> int:
    ...
    output = Path(sys.argv[1] if len(sys.argv) > 1 else "akron-council-minutes-source-contract")
    ...
    if urlparse(endpoint).hostname != "onlinedocs.akronohio.gov":
        raise ValueError("Council minutes keyword endpoint escaped publisher host")

    request_payload = {"QueryID": int(query["id"])}
    response, raw = post_json(endpoint, request_payload)
    (output / "council-minutes-keywords.json").write_bytes(raw)
    response.update(
        {
            "byte_length": len(raw),
            "sha256": sha256_bytes(raw),
            "same_host": urlparse(response["requested_url"]).hostname
            == urlparse(response["final_url"]).hostname,
        }
    )
    if not response.get("ok") or not response.get("same_host"):
        raise RuntimeError("publisher Council Meeting Minutes keyword metadata was unavailable")

    _, keywords = parse_keywords(raw)
    payload = {
        "schema": SCHEMA,
        "stage": "publisher_council_minutes_query_metadata_only",
        "source_contract": {
            "schema": source["schema"],
            "api_root": api_root,
            "query": query,
            "query_signature_sha256": sha256_json(query),
        },
        "keyword_metadata": {
            "endpoint": endpoint,
        ...
        "outcome": {
            "status": "unknown",
            "reason": "This stage requests publisher query metadata only and does not search Council Meeting Minutes documents.",
        },
        "non_claims": [
            "Council Meeting Minutes query metadata does not establish that minutes exist for any particular meeting.",
            "No Eastwood search is submitted in this stage.",
            "No absence, candidate, meeting minute, or terminal disposition is inferred from metadata."
        ],
    }
    (output / "council-minutes-source-contract.json").write_text(
