...
SCRIPT = EXPERIMENT / "probe_t21_council_minutes_source_contract.py"

spec = importlib.util.spec_from_file_location("t21_council_minutes_contract", SCRIPT)
...
def load_source() -> dict:
    return json.loads(
        (EXPERIMENT / "r1_t21_public_access_source_contract_summary.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_select_minutes_query_uses_frozen_publisher_identity() -> None:
    source = load_source()
    query = module.select_minutes_query(source)
